...
import matplotlib
import torch
import lanfos.eval.colors as colors
...
def apply_colormap(image, colormap_options: ColormapOptions = ColormapOptions(), eps: float = 1e-9):
    """
...
def apply_float_colormap(image, colormap: Colormaps = "viridis"):
...
def apply_depth_colormap(depth, accumulation = None, near_plane: Optional[float] = None,
                         far_plane: Optional[float] = None, colormap_options: ColormapOptions = ColormapOptions()):
...
def apply_boolean_colormap(image, true_color = colors.WHITE, false_color = colors.BLACK):
    """Converts a depth image to color for easier analysis.
