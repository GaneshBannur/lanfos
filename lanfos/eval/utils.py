...
from lanfos.eval.colormaps import apply_colormap
...
def colormap_saving(image: torch.Tensor, colormap_options, save_path):
    ...
    if save_path is not None:
        if(cv2.imwrite(save_path, cv2.cvtColor(output_image, cv2.COLOR_RGB2BGR))==False):
                print(f'Failed to save mask at path: {save_path}')
    return output_image
